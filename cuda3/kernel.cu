...
__global__ void 
vbin2(uint8_t* src, uint8_t* dst, const int width_in, const int height_in)
{
    int col0 = blockDim.x*blockIdx.x + threadIdx.x;
    if (col0 >= width_in) {
        return;
    ...
    int yidx = 0;
    int dstyidx = 0;
    for (int y = 0; y < height_in; y += 2,dstyidx++ ) {
        //vbin
        int ysum = src[y*width_in] + src[(y+1)*width_in];
        dst[dstyidx * width_in + dstyidx ] = ysum;
    }

}
__global__ void 
vbin4(uint8_t* src, uint8_t* dst, const int width_in, const int height_in)
{
    ...
    int yidx = 0;
    int dstyidx = 0;
    for (int y = 0; y < height_in; y += 4,dstyidx++ ) {
        //vbin
        int ysum = src[y*width_in + x] + 
            src[(y+1)*width_in + x] + 
            src[(y+2)*width_in + x] +
            src[(y+3)*width_in + x];
        dst[dstyidx * width_in + dstyidx ] = ysum;
...
__global__ void 
offset(uint8_t* src, uint8_t* dst, uint8_t *para, const int width_in, const int height_in)
{
    int x = blockDim.x*blockIdx.x + threadIdx.x;
    if (x >= width_in) {
        return;
    ...
    for (int y = 0; y < height_in; y ++ ) {
        //vbin
        int ysum = (int)src[y * width_in + x] - (int)para[y * width_in + x];
        if (ysum < 0) {
            ysum = 0;
        }
        dst[y * width_in + x ] = ysum;
    }

}


void vbin4_cpu(uint8_t* src, uint8_t* dst, const int width_in, const int height_in)
{
    int yidx = 0;
    int dstyidx = 0;
    for (int y = 0; y < height_in; y += 4,dstyidx++ ) {
        uint8_t* src0 = src + (y + 0) * width_in;
        uint8_t* src1 = src + (y + 1) * width_in;
        uint8_t* src2 = src + (y + 2) * width_in;
        uint8_t* src3 = src + (y + 3) * width_in;
        uint8_t* dstv = dst + dstyidx * width_in;
        for (int x = 0; x < width_in; x++) {
            //vbin
            dstv[x] = src0[x] + src1[x] + src2[x] + src3[x];
        }
    }

}

void vbin2_cpu(uint8_t* src, uint8_t* dst, const int width_in, const int height_in)
{
    int yidx = 0;
    int dstyidx = 0;
    for (int y = 0; y < height_in; y += 2,dstyidx++ ) {
